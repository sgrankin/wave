...
- 🏃 Auth methods (credential store + AuthMethod registry + MintPolicy + GitHub + OIDC) —
  in progress (background worktree agent).
...
- **critical** Bound the WaveMap: LRU/TTL eviction + cached inbox-digest projection +
  paginated inbox query (today queryapi pins every inbox wave in a never-evicting cache on
  a 5s poll with snapshots off → unbounded RAM).
...
## Tests (remaining)
- **high** comment-sheet full-height/scroll/footer geometry under a coarse/touch context.
- medium: remote-caret selection highlight + multi-peer/clamp paths; sqlite 0% funcs
